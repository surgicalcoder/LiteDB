...
public class Bson_Tests
{
    ...
    public void Bson_Using_UTC_Local_Dates()
    {
        ...
        var local = BsonSerializer.Deserialize(bytes);
    ...
    public void Bson_Partial_Deserialize()
    {
        ...
        var doc1 = BsonSerializer.Deserialize(bson, false, new HashSet<string>(new[] { "_id", "FirstString" }));
        ...
        var doc2 = BsonSerializer.Deserialize(bson, false, new HashSet<string>(new[] { "Address", "Date" }));
        ...
        var doc3 = BsonSerializer.Deserialize(bson, false, new HashSet<string>(new[] { "Last" }));

        doc3["Last"].AsInt32.Should().Be(src["Last"].AsInt32);

        // read all document
        var doc4 = BsonSerializer.Deserialize(bson);
