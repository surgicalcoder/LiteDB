...
public class Index_Tests
{
    ...
    public void EnsureIndex_Invalid_Arguments()
    {
        ...
        // null name
        {
            var exn = Assert.Throws<ArgumentNullException>(() => test.EnsureIndex(null, "x"));
            Assert.Equal("name", exn.ParamName);
        }

        // null expression 1
        {
            var exn = Assert.Throws<ArgumentNullException>(() => test.EnsureIndex(null));
            Assert.Equal("expression", exn.ParamName);
        }

        // null expression 2
        {
            var exn = Assert.Throws<ArgumentNullException>(() => test.EnsureIndex("x", null));
            Assert.Equal("expression", exn.ParamName);
        }
    }
