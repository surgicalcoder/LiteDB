...
/// <summary>
/// Compile and execute simple expressions using BsonDocuments. Used in indexes and updates operations. See
/// https://github.com/mbdavid/LiteDBX/wiki/Expressions
/// </summary>
internal class BsonExpressionParser
{
    ...
    public static BsonExpression ParseFullExpression(Tokenizer tokenizer, ExpressionContext context, BsonDocument parameters, DocumentScope scope)
    {
        ...
        while (!tokenizer.EOF)
        {
            ...
            {
                var and = tokenizer.ReadToken().Expect("AND");
    ...
    public static BsonExpression ParseSelectDocumentBuilder(Tokenizer tokenizer, ExpressionContext context, BsonDocument parameters)
    {
        ...
        bool stop(Token t)
        {
            return t.Is("FROM") || t.Is("INTO") || t.Type == TokenType.EOF || t.Type == TokenType.SemiColon;
        }
    ...
    {
        if (tokenizer.Current.Type != TokenType.OpenBrace)
        {
            return null;
        }
            ...
            while (!tokenizer.CheckEOF())
            {
                ...
                else
                {
                    ...
                    value = new BsonExpression
                    {
                        ...
                        UseSource = useSource,
                        IsScalar = true,
                        Fields = new HashSet<string>(StringComparer.OrdinalIgnoreCase).AddRange(new[] { key }),
    ...
    private static Expression ParsePath(Tokenizer tokenizer, Expression expr, ExpressionContext context, BsonDocument parameters, HashSet<string> fields, ref bool isImmutable, ref bool useSource, ref bool isScalar, StringBuilder src)
    {
        ...
        if (ahead.Type == TokenType.OpenBracket) // array 
        {
            src.Append("[");
    ...
    {
        var values = new[] { item0.Expression, item1.Expression };
    ...
    {
        var src = expr.Type == BsonExpressionType.Path ? expr.Source + "[*]" : "ITEMS(" + expr.Source + ")";

        var exprType = expr.Type == BsonExpressionType.Path ? BsonExpressionType.Path : BsonExpressionType.Call;
    ...
    internal static BsonExpression CreateLogicExpression(BsonExpressionType type, BsonExpression left, BsonExpression right)
    {
        ...
        var expr = type == BsonExpressionType.And ? Expression.AndAlso(boolLeft, boolRight) : Expression.OrElse(boolLeft, boolRight);
    ...
    /// </summary>
    private static readonly Dictionary<string, Tuple<string, MethodInfo, BsonExpressionType>> _operators = new()
    {
        // arithmetic
