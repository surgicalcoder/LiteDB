...
internal class BsonExpressionOperators
{
    ...
    {
        return left.Any(x => BETWEEN(collation, x, right));
    }

    public static BsonValue BETWEEN_ALL(Collation collation, IEnumerable<BsonValue> left, BsonValue right)
    {
        return left.All(x => BETWEEN(collation, x, right));
    }

    /// <summary>
    /// Test if left are in any value in right side (when right side is an array). If right side is not an array, just
    /// implement a simple Equals (=). Returns true or false
    /// </summary>
    ...
    /// <summary>
    /// Return a value from a value as document. If has no name, just return values ($). If value are not a document, do not
    /// return anything
    /// </summary>
    ...
    public static IEnumerable<BsonValue> ARRAY_FILTER(BsonValue value, int index, BsonExpression filterExpr, BsonDocument root, Collation collation, BsonDocument parameters)
    {
        ...
        else
        {
            foreach (var item in arr)
            {
                ...
                var c = filterExpr.ExecuteScalar(new[] { root }, root, item, collation);
