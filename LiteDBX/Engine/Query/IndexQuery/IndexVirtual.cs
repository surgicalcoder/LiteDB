...
internal class IndexVirtual : Index, IDocumentLookup
{
    ...
    private Dictionary<uint, BsonDocument> _cache = new();
    ...
    public BsonDocument Load(PageAddress rawId)
    {
        if (_cache == null)
        {
            throw new LiteException(0, $"OrderBy/GroupBy operation are supported only in virtual collection with less than {VIRTUAL_INDEX_MAX_CACHE} documents");
        }
