...
internal class Snapshot : IDisposable
{
    ...
    private readonly Dictionary<uint, BasePage> _localPages = new();
    ...
    public T GetPage<T>(uint pageID, bool useLatestVersion = false)
        where T : BasePage
    {
        return GetPage<T>(pageID, out var origin, out var position, out var walVersion, useLatestVersion);
    }
        ...
        where T : BasePage
    {
        ...
        page = ReadPage<T>(pageID, out origin, out position, out walVersion, useLatestVersion);
        ...
        where T : BasePage
    {
        ...
        if (_transPages.DirtyPages.TryGetValue(pageID, out var walPosition))
        {
            ...
            var buffer = _reader.ReadPage(walPosition.Position, Mode == LockMode.Write, FileOrigin.Log);
        ...
        var pos = _walIndex.GetPageIndex(pageID, useLatestVersion ? int.MaxValue : ReadVersion, out walVersion);

        if (pos != long.MaxValue)
        {
            // read page from log file
            var buffer = _reader.ReadPage(pos, Mode == LockMode.Write, FileOrigin.Log);
        ...
        else
        {
            ...
            var buffer = _reader.ReadPage(pagePosition, Mode == LockMode.Write, FileOrigin.Data);
        ...
        where T : BasePage
    {
        ...
        lock (_header)
        {
            ...
            {
                var free = GetPage<BasePage>(_header.FreeEmptyPageList, true);
