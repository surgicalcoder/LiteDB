...
internal class BasePage
{
    ...
    /// <summary>
    /// Get how many free bytes (including fragmented bytes) are in this page (content space) - Will return 0 bytes if page are
    /// full (or with max 255 items)
    /// </summary>
    public int FreeBytes => ItemsCount == byte.MaxValue ? 0 : PAGE_SIZE - PAGE_HEADER_SIZE - UsedBytes - FooterSize;
    ...
    public BufferSlice Get(byte index)
    {
        ...
        ENSURE(IsValidPos(position), "invalid segment position in index footer: {0}/{1}", this, index);
        ENSURE(IsValidLen(length), "invalid segment length in index footer: {0}/{1}", this, index);
    ...
    private BufferSlice InternalInsert(ushort bytesLength, ref byte index)
    {
        ...
        ENSURE(continuousBlocks == PAGE_SIZE - NextFreePosition - FooterSize - (isNewInsert ? SLOT_SIZE : 0), "continuousBlock must be same as from NextFreePosition");
    ...
    public IEnumerable<byte> GetUsedIndexs()
    {
        ...
        ENSURE(HighestIndex != byte.MaxValue, "if has items count, Highest index should be not empty");
