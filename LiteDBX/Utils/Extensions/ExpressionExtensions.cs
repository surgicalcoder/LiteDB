...
internal static class ExpressionExtensions
{
    ...
    private static readonly Regex _removeSelect = new(@"\.Select\s*\(\s*\w+\s*=>\s*\w+\.", RegexOptions.Compiled);
    private static readonly Regex _removeList = new(@"\.get_Item\(\d+\)", RegexOptions.Compiled);
    private static readonly Regex _removeArray = new(@"\[\d+\]", RegexOptions.Compiled);
