...
{
    private readonly Dictionary<int, long> _chunkLengths = new();
    ...
    private byte[] GetChunkData(int index)
    {
        ...
        var result = chunk?["data"].AsBinary;
    ...
    private void SetReadStreamPosition(long newPosition)
    {
        ...
        var loadedChunk = _currentChunkIndex;
        var newChunkIndex = 0;

        while (seekStreamPosition <= _streamPosition)
        {
            if (_chunkLengths.TryGetValue(newChunkIndex, out var length))
            {
