...
public class LiteQueryable<T> : ILiteQueryable<T>
{
    ...
    {
        return OrderBy(keySelector, Query.Descending);
    }
    ...
    /// <summary>
    /// Execute query locking collection in write mode. This is avoid any other thread change results after read document and
    ...
    /// <summary>
    /// Execute query and return resultset as IEnumerable of T. If T is a ValueType or String, return values only (not
    /// documents)
    /// </summary>
    ...
    /// <summary>
    /// Returns the only document of resultset, or null if resultset are empty; this method throw an exception if there not
    /// exactly one document in the sequence
    /// </summary>
